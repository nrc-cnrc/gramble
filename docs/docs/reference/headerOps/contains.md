...
`contains` applies to the content immediately to its left, but can be [chained](chaining).
...
The `contains` operator has the same complications and cautions about negation scope as the `starts` operator, which you can read about on the [starts page](starts).
