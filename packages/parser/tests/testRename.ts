...
describe(`${path.basename(module.filename)}`, function() {
    ...
    describe('Rename(t1->t2 of epsilon)', function() {
        const grammar = Rename(Epsilon(), "t1", "t2");
        testHasTapes(grammar, []);
        testGrammar(grammar, [{}]);
    }); 

    describe('Rename(t1->t2 of null)', function() {
        const grammar = Rename(Null(), "t1", "t2");
        testHasTapes(grammar, []);
        testGrammar(grammar, []);
    }); 

    describe('Rename(t1->t2 of t1:)', function() {
        const grammar = Rename(t1(""), "t1", "t2");
        testHasTapes(grammar, ["t2"]);
        testGrammar(grammar, [{}]);
    }); 

    describe('Rename(t1->t2 of t1:hi+t2:wo)', function() {
        const grammar = Rename(Seq(t1("hi"), t2("wo")), "t1", "t2");
        testHasTapes(grammar, ["t2"]);
        testGrammar(grammar, [ {t2: "hi"}]);
    }); 

    describe('Rename(t3->t4 of t1:hello+t2:foo)', function() {
        const grammar = Rename(Seq(t1("hello"), t2("foo")), "t3", "t4");
        testHasTapes(grammar, ["t1", "t2"]);
        testGrammar(grammar, [{t1: "hello", t2: "foo"}]);
    });
        
    describe('Rename(t1->t2 of t1:hello+t5:foo)', function() {
    ...
    describe('Alt(rename(t1->t2 of t1:hello+t5:foo) | rename(t1->t3 of t1:hello+t5:foo)', function() {
    ...
    describe('Intersecting t2:hello & rename(t1->t2 of t1:hello)', function() {
    ...
    describe('Joining t2:hello ⨝ rename(t1->t2 of t1:hello)', function() {
        const grammar = Join(t2("hello"),
                             Rename(t1("hello"), "t1", "t2"));
        testGrammar(grammar, [{t2: "hello"}]);
    });

    describe('Joining rename(t1->t2 of t1:hello) ⨝ t2:hello', function() {
        const grammar = Join(Rename(t1("hello"), "t1", "t2"),
                             t2("hello"));
        testGrammar(grammar, [{t2: "hello"}]);
    });

    describe('Joining rename(t1->t2 of t1:hello+t3:foo) ⨝ t2:hello', function() {
        const grammar = Join(Rename(Seq(t1("hello"), t3("foo")), "t1", "t2"),
                             t2("hello"));
        testGrammar(grammar, [{t2: "hello", t3: "foo"}]);
    });

    describe('Joining t2:hello ⨝ rename(t1->t2 of t1:hello+t3:foo)', function() {
        const grammar = Join(t2("hello"), Rename(Seq(t1("hello"), t3("foo")), "t1", "t2"));
        testGrammar(grammar, [{t2: "hello", t3: "foo"}]);
    });

    describe('Joining t3:foo ⨝ rename(t1->t2 of t1:hello+t3:foo)', function() {
        const grammar = Join(t3("foo"), Rename(Seq(t1("hello"), t3("foo")), "t1", "t2"));
        testGrammar(grammar, [{t2: "hello", t3: "foo"}]);
    });

    describe('Joining rename(t1->t2 of t1:hello+t3:foo) ⨝ t2:hello+t3:bar', function() {
        const grammar = Join(Rename(Seq(t1("hello"), t3("foo")), "t1", "t2"),
                             Seq(t2("hello"), t3("bar")));
        testGrammar(grammar, []);
    });

    describe('Joining t2:hello+t3:bar ⨝ rename(t1->t2 of t1:hello+t3:foo)', function() {
    ...
    describe('Filtering of t2:hiwo & rename(t1->t2 of t1:hi+t2:wo)', function() {
