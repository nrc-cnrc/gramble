...
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.Empty = exports.Rep = exports.Emb = exports.Rename = exports.Drop = exports.Proj = exports.Not = exports.Filter = exports.Join = exports.Uni = exports.Seq = exports.Lit = exports.State = exports.SimpleDevEnvironment = exports.Project = void 0;
...
class Project {
    constructor(devEnv = new devEnv_1.SimpleDevEnvironment()) {
        this.devEnv = devEnv;
        this.globalNamespace = new stateMachine_1.Namespace("__GLOBAL__");
    ...
    addSheetAux(sheetName) {
        ...
        // global namespace
        const sheetNamespace = new stateMachine_1.Namespace(sheetName);
...
class HideHeader extends AtomicHeader {
    compileAndMerge(cell, namespace, devEnv, leftNeighbor) {
        ...
        if (leftNeighbor == undefined) {
            compiledCell.markError(devEnv, "Wayward hide", '"hide" has to have something to the left of it.');
            return compiledCell;
        }
        compiledCell.state = leftNeighbor;
        for (const tape of cell.text.split("/")) {
            compiledCell.state = stateMachine_1.Hide(compiledCell.state, tape.trim());
        }
        return compiledCell;
    }
}
 ...
 */
class RevealHeader extends AtomicHeader {
    compileAndMerge(cell, namespace, devEnv, leftNeighbor) {
        const compiledCell = new HeadedCellComponent(this, cell);
        if (leftNeighbor == undefined) {
            compiledCell.markError(devEnv, "Wayward reveal", '"reveal" has to have something to the left of it.');
            return compiledCell;
        }
        const tapes = cell.text.split("/").map(t => t.trim());
        compiledCell.state = stateMachine_1.Reveal(leftNeighbor, tapes);
        return compiledCell;
    }
}
...
class EqualsHeader extends JoinHeader {
    merge(leftNeighbor, state) {
        ...
        if (leftNeighbor instanceof stateMachine_1.ConcatState) {
            ...
            return stateMachine_1.Seq(...remnant, stateMachine_1.Filter(immediateLeftNeighbor, state));
        }
        return stateMachine_1.Filter(leftNeighbor, state);
    }
}
...
var HP_SUBEXPR = miniParser_1.MPDelay(() => miniParser_1.MPAlternation(HP_UNRESERVED, HP_EMBED, HP_HIDE, HP_REVEAL, HP_JOIN, HP_PARENS));
...
const HP_REVEAL = miniParser_1.MPSequence(["reveal"], () => new RevealHeader("reveal"));
...
class State {
     ...
     */
    accepting(tape, random, symbolStack) {
        return false;
    }
    randomize() {
        return this;
    ...
    *generate(random = false, maxRecursion = 4, maxChars = 1000) {
        ...
        var stateQueue = [[initialOutput, startingState]];
        var chars = 0;
        while (stateQueue.length > 0 && chars < maxChars) {
            ...
            for (const [prevOutput, prevState] of stateQueue) {
                ...
                for (const [tape, c, matched, newState] of prevState.dQuery(allTapes, tapes_1.ANY_CHAR, random, symbolStack)) {
                    if (!matched) {
 ...
 */
class ConcatState extends NAryState {
    constructor(children, indices = undefined) {
        super(children);
    ...
    successor(tape, newChild, newIndex, symbolStack) {
    ...
    getCurrentIndex(tape, symbolStack) {
...
*/
class UnionState extends NAryState {
    constructor(children) {
        super(children);
    }
    compileAux(allTapes, symbolStack, compileLevel) {
...
class FilterState extends BinaryState {
     ...
     */
    *filter(tape, target, c1, c2, random, symbolStack) {
        for (const [c1tape, c1target, c1matched, c1next] of c1.ndQuery(tape, target, random, symbolStack)) {
            /*
            if (c1tape.isTrivial) {
             ...
             */
            for (const [c2tape, c2target, c2matched, c2next] of c2.ndQuery(c1tape, c1target, false, symbolStack)) {
 ...
 */
class JoinState extends FilterState {
    successor(newChild1, newChild2) {
...
class RepetitionState extends UnaryState {
    ...
    *ndQuery(tape, target, random, symbolStack) {
        ...
        if (this.child.accepting(tape, random, symbolStack) &&
            ...
            const successor = new RepetitionState(successorChild, this.minReps, this.maxReps, this.index + 1, this.initialChild);
            for (const result of successor.ndQuery(tape, target, random, symbolStack)) {
                yield result;
            }
        }
        for (const [childTape, childText, childMatched, childNext] of this.child.ndQuery(tape, target, random, symbolStack)) {
...
class EmbedState extends State {
    ...
    getChild(random, symbolStack = undefined) {
        if (this._child == undefined) {
    ...
    *ndQuery(tape, target, random, symbolStack) {
        ...
        for (const [childchildTape, childTarget, childMatched, childNext] of child.ndQuery(tape, target, random, symbolStack)) {
...
class RenameState extends UnaryState {
    ...
    *ndQuery(tape, target, random, symbolStack) {
        ...
        for (var [childTape, childTarget, childMatched, childNext] of this.child.ndQuery(tape, target, random, symbolStack)) {
...
class MatchState extends UnaryState {
    ...
    *ndQuery(tape, target, random, symbolStack) {
        for (const [c1tape, c1target, c1matched, c1next] of this.child.ndQuery(tape, target, random, symbolStack)) {
            ...
            for (const c of c1tape.fromToken(c1tape.tapeName, c1target)) {
                ...
                if (c1buffer instanceof LiteralState) {
                    ...
                    for (const [bufTape, bufTarget, bufMatched, bufNext] of c1buffer.ndQuery(c1tape, cTarget, random, symbolStack)) {
                        if (bufMatched) {
                            c1bufMatched = true;
                        }
                    }
